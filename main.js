...
// cPanel'de statik dosyaların (index.html, logo vb.) sorunsuz okunması için tam yol tanımı
app.use(express.static(path.join(__dirname, 'public')));

// CONFIG
const CLOUDFLARE_WORKER_URL = process.env.CLOUDFLARE_WORKER_URL || 'https://yt-proxy.psoresmi.workers.dev';

// AXIOS CLIENT (CLOUDFLARE TÖNELİ)
...
app.post('/api/v1/channel/init-public', async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            message: "Veri proxy üzerinden başarıyla taşındı.",
            raw_data_preview: typeof data === 'object' ? data : String(data).substring(0, 500)
        });
    } catch (e) {
...
// Ana sayfa isteği geldiğinde direkt index.html dosyasını fırlatıyoruz
app.get('/', (req, res) => {
...
app.listen(PORT, () => {
    console.log(`🚀 Minimal Sunucu ${PORT} portunda ayakta.`);
});